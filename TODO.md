...
请帮我完成以下跨项目改动：
1. 安卓端和后端的数据备份和同步部分中, 对于booklet和essay两部分数据备份到服务器时, 目前是直接存储为json文件并将图片保存到"./static/img_storage/[模块名]/"下,为了后续能更容易实现文章的关键词模糊查询和同步/备份时的增量更新之类的, 我希望把这两部分简单的json存储方式替换为数据表存储, 图片文件保持现有逻辑.
关于这两模块的数据表我已建好,表结构参考AGENTS_DB.md中的'用户数据表'部分. (所在scheme名为user_data)
2. desktop: 
...
完成所有工作之后进行检验, 确保项目不含报错并且实现我提到的所有功能.

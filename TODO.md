...
----
1. 我希望用户数据备份这部分除了booklet和essay这两个模块数据备份, 另外也将所有应用中其他部分也一并备份并提供同步功能:
偏好设置(两部分的背景图片和座右铭)
头像, 昵称, 签名.
网络设置.
"其他"页的漫画和藏品部分不做考虑, 我之后再考虑别的方式.
追加说明, backend:写好相应的接口和数据持久化方式. 参考已有的保存方式和位置. android: 个人-数据页做好对应的ui,新增的这些合并为"偏好", 即新增同步偏好, 备份偏好等.
